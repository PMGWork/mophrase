...
- [x] フィッティング時に不連続点を考慮した補間/最適化を行う
  ...
  - 方針メモ:
    ...
    - 実装メモ（2026-02-26）:
      - 検出軸は角度変化・距離比・速度比の3系統を採用（保守的な固定閾値）。
      - 不連続検出は常時ONで適用し、UIトグルは追加しない。
      - Sketch/Graphともに分割境界単位で独立接線を使ってフィットする。
    - 確認メモ（2026-03-01）:
      - `pnpm lint` / `pnpm build` を通過。
  - 完了条件:
    - 不連続を含む入力で境界付近の破綻が減る。
    - 連続入力では従来品質を維持する。

...
4. API レイヤ（Worker）
- `worker/src/index.ts`
- 役割: `/api/llm/generate` の提供、OpenAI/Cerebras/OpenRouter/Google AI Studio への Structured Output 中継
...
flowchart LR
  ...
  Worker --> Provider["OpenAI or Cerebras or OpenRouter or Google AI Studio"]
  Worker --> LLMClient
...
- 提案品質/契約変更: `src/prompts/keyframePrompt.common.md`, `src/prompts/keyframePrompt.parallel.md`, `src/types.ts`, `src/suggestion/*`, `worker/src/index.ts`

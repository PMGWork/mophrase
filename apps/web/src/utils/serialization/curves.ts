...
import { isGraphCorner, isSketchCorner } from '../keyframeCorner';
...
function serializeKeyframes(
  ...
  for (let i = 0; i < keyframes.length - 1; i++) {
    ...
    startKeyframe.graphOut = serializeGraphHandle(
      start.graphOut,
      {
        time: resolveFiniteTime(serializedTimes[i], start.time),
        progress: startProgress,
      },
      {
        time: resolveFiniteTime(serializedTimes[i + 1], end.time),
  ...
  if (keyframes.length >= 2) {
    ...
    if (first.graphIn && !serialized[0].graphIn) {
      serialized[0].graphIn = serializeGraphHandle(
        first.graphIn,
        {
          time: resolveFiniteTime(serializedTimes[0], first.time),
          progress: progress[0] ?? 0,
        },
        {
          time: resolveFiniteTime(serializedTimes[1], second.time),
    ...
    if (last.graphOut && !serialized[lastIdx].graphOut) {
      serialized[lastIdx].graphOut = serializeGraphHandle(
        last.graphOut,
        {
          time: resolveFiniteTime(
            serializedTimes[lastIdx - 1],
            secondToLast.time,
          ),
          progress: progress[lastIdx - 1] ?? 0,
        },
        {
          time: resolveFiniteTime(serializedTimes[lastIdx], last.time),
          progress: progress[lastIdx] ?? 0,
        },
        true,
      );
    }
...
export function serializePaths(paths: Path[]): SerializedPath[] {
  return paths.map((path) => {
    ...
    const bbox =
      curves.length > 0
        ? computeBbox(curves)
        : computeBboxFromKeyframes(path.keyframes);
...
export function deserializePathKeyframes(
  ...
  const referenceEnd =
    referenceTimes[referenceTimes.length - 1] ?? referenceStart;
  ...
  for (let i = 0; i < keyframes.length - 1; i++) {
    ...
    const v1 = deserializedProgress[i + 1] ?? referenceProgress[i + 1] ?? v0;
    const dv = v1 - v0;
    const segmentDiag = Math.hypot(dt, dv);
  ...
  if (keyframes.length >= 2) {
    ...
    if (firstSerialized?.graphIn && first && !first.graphIn) {
      ...
      if (second) {
        ...
        if (segDiag > 1e-6) {
          first.graphIn = deserializeGraphHandle(
            firstSerialized.graphIn,
            segDiag,
            p,
          );
        }
    ...
    if (lastSerialized?.graphOut && last && !last.graphOut) {
      ...
      if (secondToLast) {
        ...
        const v0 =
          deserializedProgress[lastIdx - 1] ??
          referenceProgress[lastIdx - 1] ??
          0;
        const v1 =
          deserializedProgress[lastIdx] ?? referenceProgress[lastIdx] ?? v0;
        const dv = v1 - v0;
        const segDiag = Math.hypot(dt, dv);
        if (segDiag > 1e-6) {
          last.graphOut = deserializeGraphHandle(
            lastSerialized.graphOut,
            segDiag,
            p,
          );
        }
...
function stabilizeDeserializedTimes(
  ...
  for (let i = 0; i < keyframes.length; i++) {
    ...
    const fallbackTime = Number.isFinite(fallbackReference)
...
function normalizePathTimes(keyframes: Keyframe[]): number[] {
  ...
  if (!Number.isFinite(span) || Math.abs(span) < 1e-9) {
    return baseTimes.map(
      (_, index) => index / Math.max(1, baseTimes.length - 1),
    );
  }
...
function computeBboxFromKeyframes(
  keyframes: Keyframe[],
): SerializedBoundingBox {
